  ...
  group('MutableDocument', () {
    group('.moveNode()', () {
      test('when the document is empty, throws an exception', () {
        ...
        expect(() => document.moveNode(nodeId: 'does-not-exist', targetIndex: 0), throwsException);
      });
      ...
      test('when the given target index is negative, throws a RangeError', () {
        ...
        expect(() => document.moveNode(nodeId: 'move-me', targetIndex: -1), throwsRangeError);
      });
      ...
      test('when the given target index is out of document bounds, throws a RangeError', () {
        ...
        expect(() => document.moveNode(nodeId: 'move-me', targetIndex: 3), throwsRangeError);
      });

      test(
        'when the node exists in the document, and the targetIndex is valid, moves it to the given target index',
        () {
          final node = ParagraphNode(id: 'move-me', text: AttributedText());
          final document = MutableDocument(
            nodes: [
    ...
    test(
      ...
      () {
        final document1 = MutableDocument(
          nodes: [TextNode(id: '1', text: AttributedText('a', AttributedSpans()))],
        );
        final document2 = MutableDocument(
          nodes: [TextNode(id: '1', text: AttributedText('a', AttributedSpans()))],
        );

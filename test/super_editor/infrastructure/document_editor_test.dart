...
void main() {
  group('MutableDocument', () {
    group('.moveNode()', () {
      ...
      test(
        'when the node does not exist in the document, throws an exception',
        () {
      ...
      test(
        'when the given target index is out of document bounds, throws a RangeError',
        () {
          final node = ParagraphNode(id: 'move-me', text: AttributedText());
          final document = MutableDocument(
            nodes: [
    ...
    test(
      'it is NOT equal to a document when corresponding nodes are NOT equal',
      () {
        final document1 = MutableDocument(nodes: [HorizontalRuleNode(id: '1')]);

...
void main() {
  group('SuperEditor gestures', () {
    testWidgetsOnAllPlatforms(
      'places the caret when tapping an empty document (top left)',
      (tester) async {
        ...
        await tester.tapAt(
          tester.getTopLeft(find.byType(SuperEditor)) + const Offset(1, 1),
        );
    ...
    testWidgetsOnAllPlatforms(
      'places the caret when tapping an empty document (top right)',
      (tester) async {
        final testContext = await tester
            .createDocument() //
            .withSingleEmptyParagraph()
        ...
        // Tap inside SuperEditor
        await tester.tapAt(
          tester.getTopRight(find.byType(SuperEditor)) + const Offset(-1, 1),
        );
        await tester.pump(kTapMinTime);
    ...
    testWidgetsOnAllPlatforms(
      'places the caret when tapping an empty document (bottom left)',
      (tester) async {
        final testContext = await tester
            .createDocument() //
            .withSingleEmptyParagraph()
        ...
        // Tap inside SuperEditor
        await tester.tapAt(
          tester.getBottomLeft(find.byType(SuperEditor)) + const Offset(1, -1),
        );
        await tester.pump(kTapMinTime);
    ...
    testWidgetsOnAllPlatforms(
      'places the caret when tapping an empty document (bottom right)',
      (tester) async {
        final testContext = await tester
            .createDocument() //
            .withSingleEmptyParagraph()
        ...
        // Tap inside SuperEditor
        await tester.tapAt(
          tester.getBottomRight(find.byType(SuperEditor)) - const Offset(1, 1),
        );
        await tester.pump(kTapMinTime);
    ...
    testWidgetsOnAllPlatforms(
      'places the caret when tapping an empty document (center)',
      (tester) async {
        final testContext = await tester
            .createDocument() //
            .withSingleEmptyParagraph()
    ...
    testWidgetsOnAllPlatforms(
      'places the caret at the end when tapping beyond the end of the document',
      (tester) async {
        final testContext = await tester
            .createDocument() //
        ...
        await tester.tapAt(
          tester.getBottomLeft(find.byType(SuperEditor)) - const Offset(0, 10),
        );
        await tester.pump(kTapMinTime);
    ...
    testWidgetsOnAllPlatforms(
      'places the caret at the beginning when tapping above the start of the content',
      (tester) async {
        final testContext = await tester
            .createDocument() //
            .withSingleParagraph()
        ...
        await tester.tapAt(
          tester.getTopRight(find.byType(SuperEditor)) - const Offset(10, 0),
        );
        await tester.pump(kTapMinTime);
...
spans multiple lines.''')
    ...
    testWidgetsOnAllPlatforms(
      "places the caret at the end of a wrapped line when tapping there",
      (tester) async {
    ...
    testWidgetsOnAllPlatforms(
      "places the caret at the beginning of a wrapped line when tapping there",
      (tester) async {
        // Configure and render a document.
        await tester
            .createDocument()
        ...
        final offsetOfLineBreak = SuperEditorInspector.findOffsetOfLineBreak(
          '1',
        );
    ...
    testWidgetsOnDesktop('scrolls the content when dragging the scrollbar down', (
      tester,
    ) async {
      ...
      final thumbLocation =
          tester.getTopRight(find.byType(SuperEditor)) + const Offset(-10, 10);

      // Hover to make the thumb visible with a duration long enough to run the fade in animation.
      final testPointer = TestPointer(1, PointerDeviceKind.mouse);

      await tester.sendEventToBinding(
        testPointer.hover(thumbLocation, timeStamp: const Duration(seconds: 1)),
      );
      ...
      // Move the thumb down.
      await tester.sendEventToBinding(
        testPointer.move(thumbLocation + const Offset(0, 300)),
      );
      await tester.pump();
    ...
    testWidgetsOnDesktop('scrolls the content when dragging the scrollbar up', (
      ...
      // Find the approximate position of the scrollbar thumb.
      final thumbLocation =
          tester.getBottomRight(find.byType(SuperEditor)) -
          const Offset(10, 10);
    ...
    group("interaction mode", () {
      group("when active", () {
        testWidgetsOnAllPlatforms("launches URL on tap", (tester) async {
          ...
          expect(
            context.findEditContext().composer.isInInteractionMode.value,
            isTrue,
          );
          ...
          expect(
            testUrlLauncher.urlLaunchLog.first.toString(),
            "https://fake.url",
          );
        });

        testWidgetsOnAllPlatforms("launches different URLs on tap", (
          tester,
        ) async {
          // Setup test version of UrlLauncher to log URL launches.
          final testUrlLauncher = TestUrlLauncher();
          ...
          final context =
              await tester //
                  .createDocument()
                  .fromMarkdown(
                    "[Google](https://google.com) and [Flutter](https://flutter.dev)",
                  )
                  .autoFocus(true)
                  .pump();
          ...
          expect(
            testUrlLauncher.urlLaunchLog.last.toString(),
            "https://flutter.dev",
          );
        });
      });

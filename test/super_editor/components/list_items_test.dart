  ...
  group('List items', () {
    group('node conversion', () {
      testWidgetsOnArbitraryDesktop(
        "applies styles when unordered list item is converted to and from a paragraph",
        (WidgetTester tester) async {
          ...
          richText =
              (find.byType(LayoutAwareRichText).evaluate().first.widget)
                  as LayoutAwareRichText;
      ...
      testWidgetsOnArbitraryDesktop(
        "applies styles when ordered list item is converted to and from a paragraph",
        (WidgetTester tester) async {
    ...
    group('newlines >', () {
      testWidgetsOnAllPlatforms(
        "does nothing when caret is in non-deletable task",
        (tester) async {
          await tester
              .createDocument()
              .withCustomContent(
          ...
          expect(
            document.first.asTextNode.text.toPlainText(),
            "Non-deletable list item.",
          );
      ...
      testWidgetsOnAllPlatforms(
        "does nothing when non-deletable content is selected",
        (tester) async {
    ...
    group('unordered list', () {
      testWidgetsOnDesktop('updates caret position when indenting', (
        tester,
      ) async {
        ...
        final caretOffsetBeforeIndent =
            SuperEditorInspector.findCaretOffsetInDocument();
        final firstCharacterRectBeforeIndent =
            SuperEditorInspector.findDocumentLayout().getRectForPosition(
              DocumentPosition(
                nodeId: doc.first.id,
        ...
        expect(
          caretOffsetBeforeIndent.dx,
          moreOrLessEquals(firstCharacterRectBeforeIndent.left, epsilon: 5),
        );
        ...
        // We only care about a roughly accurate caret offset because the logic around
        // exact caret positioning might change and we don't want that to break this test.
        final caretOffsetAfterIndent =
            SuperEditorInspector.findCaretOffsetInDocument();
        expect(
          caretOffsetAfterIndent.dx,
          greaterThan(caretOffsetBeforeIndent.dx),
        );
        final firstCharacterRectAfterIndent =
            SuperEditorInspector.findDocumentLayout().getRectForPosition(
              DocumentPosition(
                nodeId: doc.first.id,
                nodePosition: const TextNodePosition(offset: 0),
        ...
        expect(
          caretOffsetAfterIndent.dx,
          moreOrLessEquals(firstCharacterRectAfterIndent.left, epsilon: 5),
        );
      });

      testWidgetsOnDesktop('updates caret position when unindenting', (
        tester,
      ) async {
        ...
        // We only care about a roughly accurate caret offset because the logic around
        // exact caret positioning might change and we don't want that to break this test.
        final caretOffsetBeforeUnIndent =
            SuperEditorInspector.findCaretOffsetInDocument();
        final firstCharacterRectBeforeUnIndent =
            SuperEditorInspector.findDocumentLayout().getRectForPosition(
              DocumentPosition(
                nodeId: doc.last.id,
                nodePosition: const TextNodePosition(offset: 0),
        ...
        expect(
          caretOffsetBeforeUnIndent.dx,
          moreOrLessEquals(firstCharacterRectBeforeUnIndent.left, epsilon: 5),
        );
        ...
        // We only care about a roughly accurate caret offset because the logic around
        // exact caret positioning might change and we don't want that to break this test.
        final caretOffsetAfterUnIndent =
            SuperEditorInspector.findCaretOffsetInDocument();
        expect(
          caretOffsetAfterUnIndent.dx,
          lessThan(caretOffsetBeforeUnIndent.dx),
        );
        final firstCharacterRectAfterUnIndent =
            SuperEditorInspector.findDocumentLayout().getRectForPosition(
              DocumentPosition(
                nodeId: doc.last.id,
                nodePosition: const TextNodePosition(offset: 0),
        ...
        expect(
          caretOffsetAfterUnIndent.dx,
          moreOrLessEquals(firstCharacterRectAfterUnIndent.left, epsilon: 5),
        );
      });
      ...
      testWidgetsOnDesktopAndWeb(
        'unindents with BACKSPACE with caret at beginning of list item',
        (tester) async {
          await _pumpUnorderedListWithTextField(tester);
      ...
      testWidgetsOnAllPlatforms(
        "inserts new item on ENTER at end of existing item",
        (tester) async {
          final context =
              await tester //
                  .createDocument()
          ...
          expect(document.first, isA<ListItemNode>());
          expect(
            (document.first as ListItemNode).text.toPlainText(),
            "Item 12",
          );
      ...
      testWidgetsOnAndroid(
        "inserts new item upon new line insertion at end of existing item",
        (tester) async {
          final context =
              await tester //
                  .createDocument()
      ...
      testWidgetsOnMobile(
        "inserts new item upon new line input action at end of existing item",
        (tester) async {
          final context =
              await tester //
                  .createDocument()
      ...
      testWidgetsOnAllPlatforms(
        "splits list item into two on ENTER in middle of existing item",
        (tester) async {
          final context =
              await tester //
                  .createDocument()
      ...
      testWidgetsOnAndroid(
        "splits list item into two upon new line insertion in middle of existing item",
        (tester) async {
          final context =
              await tester //
                  .createDocument()
      ...
      testWidgetsOnMobile(
        "splits list item into two upon new line input action in middle of existing item",
        (tester) async {
          final context =
              await tester //
                  .createDocument()
    ...
    group('ordered list', () {
      testWidgetsOnArbitraryDesktop(
        'keeps sequence for items split by unordered list',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
...
2. Second ordered item
   ...
   - Second unoredered item""") //
          ...
          expect(
            (context.document.getNodeAt(0) as ListItemNode).type,
            ListItemType.ordered,
          );

          expect(context.document.getNodeAt(1), isA<ListItemNode>());
          expect(
            (context.document.getNodeAt(1) as ListItemNode).type,
            ListItemType.unordered,
          );

          expect(context.document.getNodeAt(2), isA<ListItemNode>());
          expect(
            (context.document.getNodeAt(2) as ListItemNode).type,
            ListItemType.unordered,
          );

          expect(context.document.getNodeAt(3), isA<ListItemNode>());
          expect(
            (context.document.getNodeAt(3) as ListItemNode).type,
            ListItemType.ordered,
          );

          expect(context.document.getNodeAt(4), isA<ListItemNode>());
          expect(
            (context.document.getNodeAt(4) as ListItemNode).type,
            ListItemType.unordered,
          );

          expect(context.document.getNodeAt(5), isA<ListItemNode>());
          expect(
            (context.document.getNodeAt(5) as ListItemNode).type,
            ListItemType.unordered,
          );
          ...
          final firstOrderedItem = tester.widget<OrderedListItemComponent>(
            find.ancestor(
              of: find.byWidget(
                SuperEditorInspector.findWidgetForComponent(
          ...
          final secondOrderedItem = tester.widget<OrderedListItemComponent>(
            find.ancestor(
              of: find.byWidget(
                SuperEditorInspector.findWidgetForComponent(
                  context.document.getNodeAt(3)!.id,
                ),
              ),
              matching: find.byType(OrderedListItemComponent),
            ),
      ...
      testWidgetsOnArbitraryDesktop(
        'keeps sequence for items split by ordered list items with higher indentation',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
...
""") //
          ...
          for (int i = 0; i < 6; i++) {
            ...
            expect(
              (context.document.getNodeAt(i) as ListItemNode).type,
              ListItemType.ordered,
            );
          }
          ...
          // Ensure the sequence was kept.
          expect(
            SuperEditorInspector.findListItemOrdinal(
              context.document.getNodeAt(0)!.id,
            ),
            1,
          );
          expect(
            SuperEditorInspector.findListItemOrdinal(
              context.document.getNodeAt(1)!.id,
            ),
            2,
          );
          expect(
            SuperEditorInspector.findListItemOrdinal(
              context.document.getNodeAt(2)!.id,
            ),
            1,
          );
          expect(
          ...
          expect(
            SuperEditorInspector.findListItemOrdinal(
              context.document.getNodeAt(4)!.id,
            ),
            3,
          );
          expect(
            SuperEditorInspector.findListItemOrdinal(
              context.document.getNodeAt(5)!.id,
            ),
            1,
          );
        },
      ...
      testWidgetsOnArbitraryDesktop(
        'restarts item order when separated by an unordered item',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
...
2. Second ordered item""") //
      ...
      testWidgetsOnArbitraryDesktop(
        'does not keep sequence for items split by paragraphs',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
...
2. Second ordered item""") //
      ...
      testWidgetsOnArbitraryDesktop('updates caret position when indenting', (
        tester,
      ) async {
        await _pumpOrderedListWithTextField(tester);
      ...
      testWidgetsOnArbitraryDesktop('updates caret position when unindenting', (
        tester,
      ) async {
        await _pumpOrderedListWithTextField(tester);
...
TextStyle _inlineTextStyler(Set<Attribution> attributions, TextStyle base) =>
    base;
...
final _styleSheet = Stylesheet(
  ...
  rules: [
    StyleRule(const BlockSelector("paragraph"), (doc, docNode) {
      return {
        Styles.textStyle: const TextStyle(color: Colors.red, fontSize: 16),
      };
    }),

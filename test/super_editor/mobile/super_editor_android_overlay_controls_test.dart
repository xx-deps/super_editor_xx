...
void main() {
  group("SuperEditor > Android > overlay controls >", () {
    testWidgetsOnAndroid("hides all controls when placing the caret", (
      tester,
    ) async {
    ...
    testWidgetsOnAndroid("shows magnifier when dragging the caret", (
      tester,
    ) async {
      await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid("shows magnifier when dragging the collapsed handle", (
      tester,
    ) async {
      await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid(
      "shows and hides toolbar upon tap on collapsed handle",
      (tester) async {
        await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid("hides toolbar when the user taps to move the caret", (
      tester,
    ) async {
      await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid("does not show toolbar upon first tap", (
      tester,
    ) async {
      await tester //
          .createDocument()
    ...
    testWidgetsOnAndroid("shows toolbar when selection is expanded", (
      tester,
    ) async {
      await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid("hides toolbar when tapping on expanded selection", (
      tester,
    ) async {
      await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid("shows magnifier when dragging expanded handle", (
      tester,
    ) async {
      await _pumpSingleParagraphApp(tester);
    ...
    testWidgetsOnAndroid(
      "shows expanded handles when dragging to a collapsed selection",
      (tester) async {
        await _pumpSingleParagraphApp(tester);
        ...
        await gesture.moveBy(
          SuperEditorInspector.findDeltaBetweenCharactersInTextNode('1', 0, 5),
        );
        await tester.pump();
        ...
        expect(
          SuperEditorInspector.findAllMobileDragHandles(),
          findsExactly(2),
        );
        expect(
          tester.getTopLeft(
          ...
          offsetMoreOrLessEquals(
            documentLayout.getGlobalOffsetFromDocumentOffset(
                  selectedPositionRect.bottomRight,
                ) -
                Offset(
                  AndroidSelectionHandle.defaultTouchRegionExpansion.left,
                  0,
                ),
          ),
        );
        expect(
          ...
          offsetMoreOrLessEquals(
            documentLayout.getGlobalOffsetFromDocumentOffset(
                ...
                Offset(
                  AndroidSelectionHandle.defaultTouchRegionExpansion.right,
                  0,
                ),
          ),
        );
    ...
    testWidgetsOnAndroid("shows expanded handles when expanding the selection", (
      tester,
    ) async {
    ...
    testWidgetsOnAndroid("allows customizing the collapsed handle", (
      tester,
    ) async {
    ...
    testWidgetsOnAndroid("allows customizing the expanded handles", (
      tester,
    ) async {
    ...
    group('shows magnifier above the caret when dragging the collapsed handle', () {
      testWidgetsOnAndroid('with an ancestor scrollable', (tester) async {
        ...
        expect(
          tester.getTopLeft(SuperEditorInspector.findMobileCaret()).dy -
              tester
                  .getBottomLeft(SuperEditorInspector.findMobileMagnifier())
                  .dy,
          lessThan(20.0),
        );
      ...
      testWidgetsOnAndroid(
        'without an ancestor scrollable having widgets above the editor',
        (tester) async {
          final scrollController = ScrollController();
    ...
    group("on device and web > shows", () {
      ...
      testWidgetsOnAndroidDeviceAndWeb("upstream and downstream handles", (
        tester,
      ) async {
        await _pumpSingleParagraphApp(tester);

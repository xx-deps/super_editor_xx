...
void main() {
  group("SuperEditor action tags >", () {
    group("composing >", () {
      ...
      testWidgetsOnAllPlatforms("can start at the beginning of a word", (
        tester,
      ) async {
        await _pumpTestEditor(
          tester,
        ...
        final spans = SuperEditorInspector.findTextInComponent("1")
      ...
      testWidgetsOnAllPlatforms(
        "can be configured to use a different trigger",
        (tester) async {
          await _pumpTestEditor(
            tester,
            MutableDocument(
      ...
      testWidgetsOnAllPlatforms(
        "does not continue when user expands the selection upstream",
        (tester) async {
          await _pumpTestEditor(
            tester,
            MutableDocument(
      ...
      testWidgetsOnAllPlatforms(
        "does not continue when user expands the selection downstream",
        (tester) async {
          await _pumpTestEditor(
            tester,
            MutableDocument(
      ...
      testWidgetsOnDesktop(
        "cancels composing when deleting the trigger character",
        (tester) async {
          await _pumpTestEditor(
            tester,
            MutableDocument(
          ...
          expect(
            SuperEditorInspector.findTextInComponent(
              ...
              attributionFilter: (candidate) =>
                  candidate == actionTagComposingAttribution,
              range: const SpanRange(0, 13),
            ),
            isEmpty,
          );
      ...
      testWidgetsOnMobile(
        "cancels composing when deleting the trigger character with software keyboard",
        (tester) async {
          await _pumpTestEditor(
            tester,
            MutableDocument(
      ...
      testWidgetsOnAllPlatforms("does not re-apply a canceled tag", (
        tester,
      ) async {
        await _pumpTestEditor(
          tester,
  ...
  group("selections >", () {
    testWidgetsOnArbitraryDesktop(
      'does not extract a tag when the selection is expanded',
      (tester) async {
        await _pumpTestEditor(
          tester,
          MutableDocument(
            nodes: [
              ...
              ParagraphNode(
                id: '2',
                text: AttributedText('Another paragraph with longer text'),
              ),
            ],
          ),
        );
        ...
        final firstParagraphText = SuperEditorInspector.findTextInComponent(
          "1",
        );
        expect(
        ...
        final secondParagraphText = SuperEditorInspector.findTextInComponent(
          "1",
        );
        expect(
    ...
    testWidgetsOnAllPlatforms(
      "does not extract a tag when expanding the selection from a non-text node",
      (tester) async {

  ...
  group("SuperEditor content conversion >", () {
    ...
    group("paragraph to ordered list >", () {
      ...
      testWidgetsOnAllPlatforms(
        'with a number that continues the sequence',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
...
''')
          ...
          await tester.typeImeText(
            _orderedListNumberVariant.currentValue!.replaceAll('n', '4'),
          );
      ...
      testWidgetsOnAllPlatforms(
        'does not convert with a number that does not continues the sequence',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
...
''')
          ...
          final orderedListItemPattern = _orderedListNumberVariant.currentValue!
              .replaceAll('n', '5');
          await tester.typeImeText(orderedListItemPattern);
          ...
          expect(
            (editingNode as ParagraphNode).text.toPlainText(),
      ...
      testWidgetsOnAllPlatforms(
        'does not start a list with a number bigger than one',
        (tester) async {
          final context =
              await tester //
                  .createDocument()
    ...
    group("paragraph to horizontal rule >", () {
      testAllInputsOnAllPlatforms(
        "with --- at the beginning of an empty paragraph",
        (tester, {required TextInputSource inputSource}) async {
          ...
          expect(
            (document.last as ParagraphNode).text.toPlainText().isEmpty,
            isTrue,
          );
        },
      );
      ...
      testAllInputsOnAllPlatforms(
        'with --- at the beginning of an non-empty paragraph',
        (tester, {required TextInputSource inputSource}) async {
          final context =
              await tester //
                  .createDocument()
          ...
          expect(
            (context.document.first as ParagraphNode).text.toPlainText(),
            '-Existing paragraph',
          );
          ...
          expect(
            (context.document.last as ParagraphNode).text.toPlainText(),
            'Existing paragraph',
          );
        },
      );
      ...
      testWidgetsOnAllPlatforms('does not convert non-HR dashes', (
        ...
        expect(paragraphNode, isA<ParagraphNode>());
        expect(
          (paragraphNode as ParagraphNode).text.toPlainText(),
          expectedResult,
        );

...
void main() {
  group("SuperTextField Android selection >", () {
    testWidgetsOnAndroid("long-pressing in empty space shows the toolbar", (
      tester,
    ) async {
    ...
    testWidgetsOnAndroid(
      "long-pressing in empty space when there is NO selection does NOT show the toolbar",
      (tester) async {
        await _pumpTestApp(tester);
    ...
    testWidgetsOnAndroid(
      "tapping at collapsed handle shows/hides the toolbar",
      (tester) async {
        await _pumpTestApp(tester);
    ...
    testWidgetsOnAndroid(
      "tapping at existing collapsed selection shows/hides the toolbar",
      (tester) async {
        await _pumpTestApp(tester);
    ...
    testWidgetsOnAndroid(
      "tapping at existing expanded selection places the caret",
      (tester) async {
        await _pumpTestApp(tester);
    ...
    group("drag handle selection > ", () {
      testWidgetsOnAndroid("selects by word when dragging downstream", (
        ...
        final downstreamPositionBox = textLayout.getCharacterBox(
          const TextPosition(offset: 17),
        );
        final desiredPositionBox = textLayout.getCharacterBox(
          const TextPosition(offset: 23),
        );
        final gesture = await tester
            .dragDownstreamMobileHandleByDistanceInSuperTextField(
              Offset(
                desiredPositionBox!.right - downstreamPositionBox!.right,
                0.0,
              ),
            );
      ...
      testWidgetsOnAndroid(
        ...
        (tester) async {
          ...
          final gesture = await tester
              .dragDownstreamMobileHandleByDistanceInSuperTextField(
                Offset(
                  desiredPositionBox!.left - downstreamPositionBox!.right,
                  0.0,
                ),
              );
          ...
          expect(
            SuperTextFieldInspector.findSelection(),
            const TextSelection(baseOffset: 27, extentOffset: 30),
          );
      ...
      testWidgetsOnAndroid("selects by word when dragging upstream", (
        ...
        //            ^ position 10
        final textLayout = SuperTextFieldInspector.findProseTextLayout();
        final upstreamPositionBox = textLayout.getCharacterBox(
        ...
        expect(
          SuperTextFieldInspector.findSelection(),
          const TextSelection(baseOffset: 6, extentOffset: 17),
        );
      ...
      testWidgetsOnAndroid(
        ...
        (tester) async {
          ...
          final textLayout = SuperTextFieldInspector.findProseTextLayout();
          final upstreamPositionBox = textLayout.getCharacterBox(
            const TextPosition(offset: 27),
          );
          final desiredPositionBox = textLayout.getCharacterBox(
            const TextPosition(offset: 34),
          );
          final gesture = await tester
              .dragUpstreamMobileHandleByDistanceInSuperTextField(
                Offset(
                  desiredPositionBox!.left - upstreamPositionBox!.left,
                  0.0,
                ),
              );
          ...
          expect(
            SuperTextFieldInspector.findSelection(),
            const TextSelection(baseOffset: 34, extentOffset: 38),
          );

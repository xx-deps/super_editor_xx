...
void main() {
  group('SuperTextField', () {
    group('on mobile with an ancestor Scrollable', () {
      _testWidgetsOnMobileWithKeyboard(
        'auto scrolls when focused in single-line',
        (tester, keyboardToggle) async {
          await _pumpTestApp(
            tester,
          ...
          expect(
            selectionOffset.dy.floor(),
            lessThanOrEqualTo(screenSizeWithKeyboard.height),
          );
      ...
      _testWidgetsOnMobileWithKeyboard(
        'auto scrolls when focused in multi-line',
        (tester, keyboardToggle) async {
          await _pumpTestApp(
            tester,
            text: 'This is\na multiline\nSuperTextField',
      ...
      _testWidgetsOnMobileWithKeyboard(
        'doest not auto scroll when not focused',
        (tester, keyboardToggle) async {
          await _pumpTestApp(
            tester,
            text: 'Single line SuperTextField',
          ...
          final finalTopLeft = tester.getTopLeft(
            find.byType(SuperTextField, skipOffstage: false),
          );
    ...
    testWidgetsOnAllPlatforms('auto scroll doesn\'t crash when text is empty', (
      tester,
    ) async {
      ...
      home: Scaffold(
        body: ListView(
          children: [
            ...List.generate(
              8,
              (index) => const ListTile(title: Text("BEFORE")),
            ),
            SuperTextField(
              minLines: lineCount,
              maxLines: lineCount,
...
void _testWidgetsOnMobileWithKeyboard(
  String description,
  Future<void> Function(WidgetTester tester, _KeyboardToggle keyboardToggle)
  test,
) {
...
class _KeyboardToggle {
  ...
  Future<void> open() async {
    ...
    double totalWidthResize =
        sizeWithoutKeyboard.width - sizeWithKeyboard.width;
    double totalHeightResize =
        sizeWithoutKeyboard.height - sizeWithKeyboard.height;
    ...
    for (var i = 0; i < frameCount; i++) {
      ...
      final currentScreenSize =
          (sizeWithoutKeyboard - Offset(resizedWidth, resizedHeight)) as Size;

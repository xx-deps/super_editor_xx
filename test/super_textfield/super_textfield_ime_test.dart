...
void main() {
  group('SuperTextField', () {
    group('with IME input source', () {
      group('inserts character', () {
        ...
        testWidgetsOnAllPlatforms(
          'and clears composing region after text changes',
          (tester) async {
        ...
        testWidgetsOnAllPlatforms(
          'and don\'t send editing value back to IME if matches the expected value',
          (tester) async {
            await _pumpEmptySuperTextField(tester);
            await tester.placeCaretInSuperTextField(0);
        ...
        testWidgetsOnAllPlatforms(
          'and send editing value back to IME if it doesn\'t match the expected value',
          (tester) async {
            final controller = ImeAttributedTextEditingController(
      ...
      group('inserts line', () {
        ...
        testWidgetsOnLinux('when NUMPAD ENTER is pressed in middle of text', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
        ...
        testWidgetsOnMac('when NUMPAD ENTER is pressed in middle of tex', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
        ...
        // TODO: Merge this with the testWidgetsOnMac below when Flutter supports numpad enter on windows
        testWidgetsOnLinux(
          'when NUMPAD ENTER is pressed at beginning of text',
          (tester) async {
            await _pumpSuperTextField(
              tester,
              AttributedTextEditingController(
        ...
        testWidgetsOnMac('when NUMPAD ENTER is pressed at beginning of text', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
        ...
        // TODO: Merge this with the testWidgetsOnMac below when Flutter supports numpad enter on windows
        testWidgetsOnLinux('when NUMPAD ENTER is pressed at end of text', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
        ...
        testWidgetsOnMac('when NUMPAD ENTER is pressed at end of text', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
      ...
      group('delete text', () {
        testWidgetsOnAllPlatforms('BACKSPACE does nothing when text is empty', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
        ...
        testWidgetsOnAllPlatforms('BACKSPACE deletes the previous character', (
          tester,
        ) async {
          await _pumpSuperTextField(
            tester,
      ...
      testWidgetsOnAllPlatforms(
        'clears composing region after selection changes',
        (tester) async {
          final controller = ImeAttributedTextEditingController();
          await _pumpSuperTextField(tester, controller);
          ...
          expect(
            controller.composingRegion,
            const TextRange(start: 11, end: 13),
          );
    ...
    testWidgetsOnMobile('configures the software keyboard action button', (
      tester,
    ) async {
      await tester.pumpWidget(
        _buildScaffold(
          child: const SuperTextField(textInputAction: TextInputAction.next),
        ),
      );
    ...
    testWidgetsOnAllPlatforms('disconnects from IME when disposed', (
      tester,
    ) async {
      final controller = ImeAttributedTextEditingController();
      await _pumpSuperTextField(tester, controller);
    ...
    testWidgetsOnAllPlatforms('applies custom IME configuration', (
      tester,
    ) async {
    ...
    testWidgetsOnAllPlatforms('applies viewId when attaching to the IME', (
      tester,
    ) async {
      await _pumpEmptySuperTextField(tester);
      ...
      tester
          .interceptChannel(SystemChannels.textInput.name) //
          .interceptMethod('TextInput.setClient', (methodCall) {
            final textInputConfig =
                (methodCall.arguments as List<dynamic>)[1] as Map;
    ...
    group('on iPhone 15 (iOS 17.5)', () {
      testWidgetsOnIos(
        'ignores keyboard autocorrections when pressing the action button',
        (tester) async {
          await _pumpEmptySuperTextField(tester);
  ...
  testWidgetsOnAllPlatforms('updates IME configuration when it changes', (
    tester,
  ) async {
  ...
  testWidgetsOnAllPlatforms(
    'doesn\'t re-attach to IME if the configuration doesn\'t change',
    (tester) async {
  ...
  group('SuperTextField on some bad Android software keyboards', () {
    testWidgetsOnAndroid(
      'handles BACKSPACE key event instead of deletion for a collapsed selection',
      (tester) async {
        final controller = AttributedTextEditingController(
    ...
    testWidgetsOnAndroid(
      'handles BACKSPACE key event instead of deletion for a expanded selection',
      (tester) async {
        final controller = AttributedTextEditingController(
          text: AttributedText('This is a text'),
        );
...
class _ObscuringTextController extends AttributedTextEditingController {
  ...
  void insertAtCaret({required String text, TextRange? newComposingRegion}) {
    ...
    final updatedText =
        ...
        textAfterInsertion.toPlainText().substring(
          textAfterInsertion.length - 1,
        );

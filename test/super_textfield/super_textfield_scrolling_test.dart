...
void main() {
  group("SuperTextField > scrolling >", () {
    group("single line >", () {
      testWidgetsOnAllPlatforms("scroll bar doesn't appear when empty", (
        tester,
      ) async {
      ...
      testWidgetsOnAllPlatforms(
        "scroll bar doesn't appear when viewport width slightly shrinks",
        (tester) async {
      ...
      testWidgetsOnAllPlatforms(
        "auto scrolls when the user types beyond viewport edge",
        (tester) async {
          ...
          for (int i = 0; i < textToType.length; i += 1) {
            ...
            expect(
              ...
              reason:
                  "Failed to auto-scroll on character $i - '${textToType[i]}'",
            );
          }
        },
      );
      ...
      testWidgetsOnArbitraryDesktop(
        "auto scrolls when caret moves beyond viewport edge",
        (tester) async {
          const textFieldWidth = 400.0;
      ...
      testWidgetsOnMobile("auto scrolls when caret is dragged into auto-scroll region", (
        tester,
      ) async {
        const textFieldWidth = 400.0;
        ...
        final drag = await tester.dragCaretByDistanceInSuperTextField(
          const Offset(textFieldWidth, 0),
        );
        ...
        expect(
          SuperTextFieldInspector.findScrollOffset(),
          SuperTextFieldInspector.findMaxScrollOffset(),
        );
        ...
        await tester.dragContinuation(
          drag,
          const Offset(-(textFieldWidth + 20), 0),
        );

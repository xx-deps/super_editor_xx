...
void main() {
  group('ImeAttributedTextEditingController', () {
    test('types hello **world** into empty field', () {
      final controller =
          ImeAttributedTextEditingController(
              controller: AttributedTextEditingController(selection: const TextSelection.collapsed(offset: 0)),
            )
    ...
    testWidgetsOnAllPlatforms('doesn\'t send existing IME value back to IME', (tester) async {
      ...
      final controller =
          ImeAttributedTextEditingController(
            controller: AttributedTextEditingController(text: AttributedText('Some text')),
    ...
    test('replaces selected text with new character', () {
      final controller =
          ImeAttributedTextEditingController(
              controller: AttributedTextEditingController(text: AttributedText('[replaceme]')),
            )
  ...
  testWidgets('isn\'t notified by inner controller after disposal', (tester) async {
    final innerController = AttributedTextEditingController(text: AttributedText('some text'));
    ...
    imeController = ImeAttributedTextEditingController(controller: innerController, disposeClientController: false);
    ...
    MaterialApp(
      home: Scaffold(
        body: Center(
          child: SizedBox(
            width: 300,
            child: SuperTextField(textController: controller, inputSource: inputSource),
          ),
        ),
      ),
    ),

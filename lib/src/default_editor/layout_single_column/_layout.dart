...
class _SingleColumnDocumentLayoutState extends State<SingleColumnDocumentLayout> implements DocumentLayout {
  ...
  Rect? getRectForSelection(DocumentPosition base, DocumentPosition extent) {
    ...
    if (base.nodeId == extent.nodeId) {
      ...
      final componentOffsetInDocument = (topComponent.context.findRenderObject() as RenderBox).localToGlobal(
        Offset.zero,
      ...
      final componentBoundingBox = extentComponent
          ...
          .translate(componentOffsetInDocument.dx, componentOffsetInDocument.dy);
      ...
      for (int i = 0; i < selectedNodes.length; ++i) {
        ...
        final componentOffsetInDocument = (component.context.findRenderObject() as RenderBox).localToGlobal(
          Offset.zero,
          ancestor: documentLayoutBox,
        );
        ...
        if (i == 0) {
          ...
          final selectionRectInComponent = component.getRectForSelection(startPosition, firstNodeEndPosition);
          ...
          final selectionRectInComponent = component.getRectForSelection(lastNodeStartPosition, endPosition);
          final componentRectInDocument = selectionRectInComponent.translate(
            componentOffsetInDocument.dx,
            componentOffsetInDocument.dy,
          );
  ...
  DocumentSelection? getDocumentSelectionInRegion(Offset baseOffset, Offset extentOffset) {
    ...
    if (topNodeId == bottomNodeId) {
      ...
      return DocumentSelection(
        base: DocumentPosition(nodeId: topNodeId, nodePosition: topNodeBasePosition),
        extent: DocumentPosition(nodeId: bottomNodeId, nodePosition: topNodeExtentPosition),
      );
    } else {
  ...
  DocumentPosition? _findLastPosition() {
    ...
    return DocumentPosition(nodeId: _componentKeysToNodeIds[componentKey]!, nodePosition: component.getEndPosition());
  }
...
class _PresenterComponentBuilder extends StatefulWidget {
  const _PresenterComponentBuilder({Key? key, required this.presenter, required this.watchNode, required this.builder})
    : super(key: key);
...
class _PresenterComponentBuilderState extends State<_PresenterComponentBuilder> {
  ...
  void initState() {
    ...
    _presenterListener = SingleColumnLayoutPresenterChangeListener(onViewModelChange: _onViewModelChange);
    widget.presenter.addChangeListener(_presenterListener);
  }
...
class _Component extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final componentContext = SingleColumnDocumentComponentContext(context: context, componentKey: componentKey);
      ...
      if (component != null) {
        ...
        component = ConstrainedBox(
          ...
          child: SizedBox(
            ...
            child: Padding(padding: componentViewModel.padding, child: component),
          ),
        );

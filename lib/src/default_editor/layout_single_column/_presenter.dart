...
class SingleColumnLayoutPresenter {
  ...
  void _onDocumentChange(_) {
    editorLayoutLog.finest(
      "The document changed. Marking the presenter dirty.",
    );
  ...
  void _notifyListenersOfChanges({
    ...
    for (int i = 0; i < newViewModel.componentViewModels.length; i += 1) {
      ...
      editorLayoutLog.fine(
        "Component for node $nodeId at index $i was removed",
      );
      changeMap[nodeId] = -1;
    }
...
abstract class SingleColumnLayoutComponentViewModel {
  ...
  void applyStyles(Map<String, dynamic> styles) {
    ...
    padding =
        (styles[Styles.padding] as CascadingPadding?)?.toEdgeInsets() ??
        EdgeInsets.zero;

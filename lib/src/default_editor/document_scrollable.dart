      ...
      if (includesScrollView) ...[
        Positioned(
          ...
          child: const DecoratedBox(
            decoration: BoxDecoration(color: Color(0x440088FF)),
          ),
        ),
        Positioned(
          left: 0,
...
class AutoScrollController with ChangeNotifier {
  ...
  void jumpBy(double delta) {
    ...
    scrollPosition.jumpTo(
      (scrollPosition.pixels + delta).clamp(
        0.0,
  ...
  void ensureGlobalRectIsVisible(Rect globalRegion) {
    ...
    final beyondBottomExtent = max(
      selectionExtentRectInViewport.bottom - viewportBox.size.height,
      0,
    );
    ...
    editorScrollingLog.finest(
      ' - scroll controller offset: ${scrollPosition.pixels}',
    );
    editorScrollingLog.finest(
    ...
    if (beyondTopExtent > 0) {
      newScrollPosition = (scrollPosition.pixels - beyondTopExtent).clamp(
        0.0,
        scrollPosition.maxScrollExtent,
      );

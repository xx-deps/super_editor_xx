...
class DragHandleAutoScroller {
  ...
  void ensureOffsetIsVisible(Offset offsetInViewport) {
    ...
    if (offsetInViewport.dy < _dragAutoScrollBoundary.leading) {
      ...
      editorGesturesLog.fine(
        "The scrollable needs to scroll up to make offset visible.",
      );
      ...
      editorGesturesLog.fine(
        'The scrollable needs to scroll down to make offset visible.',
      );
  ...
  void updateAutoScrollHandleMonitoring({required Offset dragEndInViewport}) {
    if (dragEndInViewport.dy < _dragAutoScrollBoundary.leading &&
      ...
      final gutterAmount = dragEndInViewport.dy.clamp(
        0.0,
        leadingScrollBoundary,
      );
      final speedPercent = (1.0 - (gutterAmount / leadingScrollBoundary)).clamp(
        0.0,
        1.0,
      );
    ...
    if (_getViewportBox().size.height - dragEndInViewport.dy <
      ...
      final gutterAmount =
          (_getViewportBox().size.height - dragEndInViewport.dy).clamp(
            0.0,
            trailingScrollBoundary,
          );

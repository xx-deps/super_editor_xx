...
class SuperEditorImageSyntax extends md.LinkSyntax {
  ...
  md.Node _tryCreateInlineLink(md.InlineParser parser, MarkdownImage link,
      {required List<md.Node> Function() getChildren}) {
    return createNode(link.destination, link.title,
  ...
  MarkdownImage? _parseInlineBracketedLink(md.InlineParser parser) {
    ...
    while (true) {
      ...
      if (char == AsciiTable.backslash) {
        ...
        final currentBuffer = buffer.toString().toLowerCase();
        if (currentBuffer.startsWith('http://') ||
            currentBuffer.startsWith('https://') ||
            currentBuffer.startsWith('ftp://')) {
          buffer.write('%20');
        } else {
          buffer.writeCharCode(char);
        }
